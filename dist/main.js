...
function onLoad() {
    ...
    mainElement === null || mainElement === void 0 ? void 0 : mainElement.prepend(productText);
...
function printHeader() {
    ...
    headerContainer === null || headerContainer === void 0 ? void 0 : headerContainer.append(pageLogo, navContainer);
...
async function searchHandler(searchValue) {
    const offsetNumber = await pHandler.getPokemonId(searchValue);
    if (offsetNumber === "NaN") {
    ...
    printModal(0, pHandler.speciesUrls[0], pHandler.pokemonObj[0].sprite, pHandler.pokemonObj[0].name);
}
async function printPokemonCards(url) {
    ...
    await pHandler.pokemonObj.forEach((pokemon) => {
        ...
        pokemonId++;
        productsContainer === null || productsContainer === void 0 ? void 0 : productsContainer.append(productCard);
...
function AddEventInfoButton() {
    ...
    allInfoButtons.forEach((infoBtn, index) => {
        infoBtn.addEventListener("click", () => {
            modalElement.style.display = "block";
            printModal(index, pHandler.speciesUrls[index], pHandler.pokemonObj[index].sprite, pHandler.pokemonObj[index].name);
        });
    });
}
async function printModal(index, speciesUrl, image, name) {
    ...
    productsHeadingElement === null || productsHeadingElement === void 0 ? void 0 : productsHeadingElement.classList.add("modalOpen");
    ...
    modalPrevPokeBtn.addEventListener("click", () => {
        ...
        modalElement.innerHTML = "";
        printModal(index - 1, pHandler.speciesUrls[index - 1], pHandler.pokemonObj[index - 1].sprite, pHandler.pokemonObj[index - 1].name);
    });
    ...
    modalElement.addEventListener("click", function handler(event) {
        if (event.target.closest(".modalBackground") != null)
            return;
        modalElement.style.display = "none";
        productsContainer.classList.remove("modalOpen");
        pagination.classList.remove("modalOpen");
...
function printPagination(url) {
    ...
    pagination === null || pagination === void 0 ? void 0 : pagination.append(btnFirstPage, btnPrevious, btnCurrent, btnNext, btnLastPage);
}
    ...
    allAddToCartButtons.forEach((cartBtn, index) => {
        cartBtn.addEventListener("click", () => {
            ...
            if (pHandler.cartItems.length == 0) {
                ...
                let i = pHandler.cartItems.findIndex((obj) => obj.pokemon.name == newPokemon.name);
                if (i !== -1) {
                    pHandler.cartItems[i].quantity += 1;
                    let totalSum = parseInt(pHandler.cartItems[i].quantity) *
